...
const ReadinessScore = () => {
  ...
  const readinessScore = responses.reduce((acc, val) => acc + val, 0);

  const handleSubmit = (e) => {
    e.preventDefault();
    alert(`Your Readiness Score is ${readinessScore}`);
  };

  return (
    <div
      className="min-h-screen bg-gradient-to-br from-black via-gray-800 to-gray-900 text-white flex flex-col items-center relative"
    >
      {/* Home Button */}
      <button
    ...
    <div>
      ...
      >
        ...
        <div className="text-center">
          <button
            type="submit"
            className="bg-green-500 px-6 py-2 rounded text-lg font-semibold shadow-md hover:bg-green-600"
          >
            Submit
          </button>
        </div>
